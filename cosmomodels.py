...
class MukhanovFirstOrder(MalikModels):
    """First order model using efold as time variable.
       y[0] - \phi_0 : Background inflaton
       y[1] - d\phi_0/d\eta : First deriv of \phi
       y[2] - H : Hubble parameter
    ...
    def __init__(self, ystart=None, tstart=0.0, tend=80.0, tstep_wanted=0.01, tstep_min=0.0001, k=None, ainit=None, solver="scipy_odeint", mass=5e-6):
        ...
        #Text for graphs
        self.plottitle = "Complex First Order Malik Model using Mukhanov variable in Efold time"
        self.tname = r"$n$"
        self.ynames = [r"$\varphi_0$",
                        r"$\dot{\varphi_0}$",
                        r"$H$",
    ...
    def derivs(self, y, t):
        ...
        #Get a
        a = self.ainit*N.exp(t)
        HdotoverH = dydx[2]/y[2]
        Aterm = (d2Udphi2 + 2*y[1]*dUdphi + (y[1])**2)/(y[2]**2)
        
        #d\deltaphi_1^prime/dn
        dydx[4] = -(1 + HdotoverH)*y[4] - ( (self.k/(a*y[2]))**2 - (2 + HdotoverH)
                    + Aterm)*y[3]
                    
        #Complex parts
        dydx[5] = y[6]
        
        dydx[6] = -(1 + HdotoverH)*y[6] - ( (self.k/(a*y[2]))**2 - (2 + HdotoverH)
                    + Aterm)*y[5]
...
class MWFirstOrder(MalikModels):
    ...
    def __init__(self, ystart=None, tstart=0.0, tend=80.0, tstep_wanted=0.01, tstep_min=0.0001, k=None, ainit=None, solver="scipy_odeint", mass=5e-6):
        ...
        #Text for graphs
        self.plottitle = "Complex First Order Malik-Wands Model in Efold time"
        self.tname = r"$n$"
        self.ynames = [r"$\varphi_0$",
                        r"$\dot{\varphi_0}$",
                        r"$H$",
    ...
    def derivs(self, y, t):
        ...
        #d\deltaphi_1^prime/dn
        dydx[4] = (-(3 + dydx[2]/y[2])*y[4] - ((self.k/(a*y[2]))**2)*y[3] 
                    -(d2Udphi2 + 2*(a**2)*y[1]*dUdphi + (a**2)*(y[1]**2)*U)*(y[3]/(y[2]**2)))
        #print dydx[4]
...
class OnlyFirstOrder(MalikModels):
    """First order Malik-Wands model using efold as time variable.
       y[0] - \phi_0 : Background inflaton
       y[1] - d\phi_0/d\eta : First deriv of \phi
       y[2] - H : Hubble parameter
    ...
    def __init__(self, ystart=None, tstart=0.0, tend=80.0, tstep_wanted=0.01, tstep_min=0.0001, k=None, ainit=None, solver="scipy_odeint", mass=5e-6, bgyresult=None):
        ...
        if bgyresult is None:
            raise ModelError("Need to provide background evolution!")
        ...
        #Initial conditions for each of the variables.
        if self.ystart is None:
            self.ystart = N.array([1.0,0.0,1.0,0.0])   
        ...
        self.plottitle = "Complex First Order Malik-Wands Model in Efold time"
        self.tname = r"$n$"
        self.ynames = [r"Real $\delta\varphi_1$",
                        r"Real $\dot{\delta\varphi_1}$",
                        r"Imag $\delta\varphi_1$",
                        r"Imag $\dot{\delta\varphi_1}$"]
    ...
    def derivs(self, y, t):
        ...
        #d\deltaphi_1^prime/dn
        dydx[1] = (-(3 + Hdot/y[2])*y[4] - ((self.k/(a*y[2]))**2)*y[3] 
                    -(d2Udphi2 + 2*(a**2)*y[1]*dUdphi + (a**2)*(y[1]**2)*U)*(y[3]/(y[2]**2)))
        #print dydx[4]
        ...
        dydx[3] = (-(3 + Hdot/y[2])*y[6] - ((self.k/(a*y[2]))**2)*y[5] 
                    -(d2Udphi2 + 2*y[1]*dUdphi + (y[1]**2)*U)*(y[5]/(y[2]**2)))
...
class MukhanovTwoStage(TwoStageModel):
    """Implementation of two stage model with standard initial conditions for phi.
    """                
    def __init__(self,  foclass=MukhanovFirstOrder, *args, **kwargs):
        """Initialize model and ensure initial conditions are sane."""
        print foclass
        #Call superclass
        super(MukhanovTwoStage, self).__init__(foclass=foclass, *args, **kwargs)
...
class EtaInitTwoStage(TwoStageModel):
    ...
    def __init__(self, *args, **kwargs):
        """Initialize model and ensure initial conditions are sane."""
        #Call superclass
        super(EtaInitTwoStage, self).__init__(*args, **kwargs)
    ...
    def getfoystart(self):
        ...
        #Set Re\delta\phi_1 initial condition
        foystart[3,:] = N.cos(-self.k*etadiff)*self.k/(astar*(N.sqrt(2)))
        #set Re\dot\delta\phi_1 ic
        foystart[4,:] = N.sin(-self.k*etadiff)*self.k*N.sqrt(self.k/2)/astar
        #Set Im\delta\phi_1
        foystart[5,:] = N.sin(-self.k*etadiff)*self.k/(astar*(N.sqrt(2)))
        #Set Im\dot\delta\phi_1
        foystart[6,:] = -N.cos(-self.k*etadiff)*self.k*N.sqrt(self.k/2)/astar
